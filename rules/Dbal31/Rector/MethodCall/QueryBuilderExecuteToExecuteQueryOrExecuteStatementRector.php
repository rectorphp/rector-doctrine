...
{
    private const string QUERY_BUILDER = 'Doctrine\DBAL\Query\QueryBuilder';
     ...
     * @var string[]
     */
    private const array STATEMENT_METHODS = [
        'insert',
        'update',
        'delete',
    ];
...
CODE_SAMPLE
    ...
    private function resolveNewMethodName(Expr $expr): ?string
    {
        while ($expr instanceof MethodCall) {
            ...
            if ($methodName !== null) {
                if (in_array($methodName, self::QUERY_METHODS, true)) {
                    return 'executeQuery';
                }

                if (in_array($methodName, self::STATEMENT_METHODS, true)) {
